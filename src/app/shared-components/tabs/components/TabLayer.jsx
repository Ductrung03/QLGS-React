...
const TabLayer = () => {
  ...
  const [selectedLayers, setSelectedLayers] = useState(['l_15', 'l_5', 'lc_861', 'lc_864', 'lc_859', 'lc_863', 'lc_865', 'lc_862']);
  ...
  const GroupItems = ({ group, icon, children, layers }) => {
    const allLayersSelected = layers && layers.length > 0 && 
    ...
    return (
        <div className="border-b last:border-b-0">
            <div
                className="flex items-center px-2 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer"
            >
                <IconButton size="small" className="mr-1" onClick={() => toggleGroup(group.id)}>
                    <FuseSvgIcon size={20}>
                        {expandedGroups.includes(group.id)
                            ? 'heroicons-outline:chevron-down'
                            : 'heroicons-outline:chevron-right'}
    ...
    {layer.children ? (
      ...
      >
        {layer.children.map(childLayer => (
          <LayerItem key={childLayer.id} layer={childLayer} indent={indent + 1} />
        ))}
      </GroupItems>
    ) : (
